...
- Имена (James LastName1, Mary LastName2, etc.) → `[MASK_<maskId>.<N>]`
- Должности (Software Engineer, Senior Developer, etc.) → `[MASK_<maskId>.<N>]`
- Отделы (Engineering #1, Marketing #2, etc.) → `[MASK_<maskId>.<N>]`
- Проекты (Project-42, Project-15, etc.) → `[MASK_<maskId>.<N>]`
...
Body: замаскированный текст из ответа на шаге Mask (например, `[MASK_<maskId>.0]`, `[MASK_<maskId>.1]`, etc.).

### Expected Unmask Response

Исходный текст восстановлен (все `[MASK_<maskId>.<N>]` заменены обратно на оригиналы). PII (`[[pii.default.N]]`) остаются заблокированными — они не восстанавливаются.
...
Выберите `model` из настроенных в `examples/config.yaml` → `routing.rules[].routes[].model`:
...
- Request body modified before sending to provider: имена → `[MASK_<maskId>.<N>]`, должности → `[MASK_<maskId>.<N>]`, отделы → `[MASK_<maskId>.<N>]`, проекты → `[MASK_<maskId>.<N>]`
- PII (email, phone, SSN) → `[[pii.default.N]]` (заблокированы, не отправляются провайдеру)
- Incident created for each detected entry
- **LLM ответ** должен содержать те же placeholders (модель видит только их, не оригиналы). Затем **unmask middleware** заменяет `[MASK_<maskId>.<N>]` обратно перед отправкой клиенту. PII остаются заблокированными.
...
2. **Mask** (`Flow A`): `POST http://localhost:8080/api/v1/shield/mask` with `Authorization: Bearer sk-test-default` and `Content-Type: text/plain` → body from section 1 (PII заблокируются, dict-значения заменятся на placeholders вида `[MASK_<maskId>.<N>]`)
...
6. **Проверка unmask в ответе LLM**: убедитесь, что ответ содержит оригинальные имена/отделы/проекты (а не `[MASK_xxx.0]`). Если видите placeholders — unmask в response path не сработал.
